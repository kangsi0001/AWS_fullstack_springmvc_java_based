...
public interface CrawlMapper {
	...
	@Insert("insert into t_actor(pidx, midx) values(#{pidx}, #{midx}) ON DUPLICATE KEY UPDATE pidx = pidx")
	void insertActor(Map<String, Object> map);
	
	@Insert("insert into t_director(pidx, midx) values(#{pidx}, #{midx}) ON DUPLICATE KEY UPDATE pidx = pidx")
	void insertDirector(Map<String, Object> map);
	
	@Insert("insert into t_cgv_attach(odr, midx) values(#{odr}, #{midx}) ON DUPLICATE KEY UPDATE midx = midx")
	...
	void updateCGV(Map<String, Object> m2);
